...
function evaluateIfIShouldLoadWaypointsFromApi(mapBounds) {
    ...
    if (isLocsLoadedInMemory()) {
        if ((mapBounds._southWest.lng > locsLoadedInMemory.swX || mapBounds._southWest.lng > locsLoadedInMemory.swY) && (mapBounds._northEast.lat < locsLoadedInMemory.neY || mapBounds._northEast.lat < locsLoadedInMemory.neX)) {
            return false;
        } else {
            return true;
        }
